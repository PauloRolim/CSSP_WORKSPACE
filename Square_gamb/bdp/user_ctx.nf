...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(user_ctx))==(mbc_board_id,OP_IMove_moveForward,OP_IMove_turn,OP_IMove_avoid_enableCollisionDetection_disableCollisionDetection,max_board_output,SimSquare_cycle_def,linear,angular);
  Inherited_List_Constants(Machine(user_ctx))==(?);
  List_Constants(Machine(user_ctx))==(mbc_board_id,OP_IMove_moveForward,OP_IMove_turn,OP_IMove_avoid_enableCollisionDetection_disableCollisionDetection,max_board_output,SimSquare_cycle_def,linear,angular)
END
&
THEORY ListSetsX IS
  Set_Definition(Machine(user_ctx),STATE)==({INIT,EXEC_1,EXEC_2,EXEC_3,FINAL});
  ...
  List_Enumerated(Machine(user_ctx))==(STATE);
  List_Defered(Machine(user_ctx))==(?);
  List_Sets(Machine(user_ctx))==(STATE)
END
&
...
&
THEORY SetsEnvX IS
  Sets(Machine(user_ctx)) == (Type(STATE) == Cst(SetOf(etype(STATE,0,4))))
END
&
THEORY ConstantsEnvX IS
  Constants(Machine(user_ctx)) == (Type(INIT) == Cst(etype(STATE,0,4));Type(EXEC_1) == Cst(etype(STATE,0,4));Type(EXEC_2) == Cst(etype(STATE,0,4));Type(EXEC_3) == Cst(etype(STATE,0,4));Type(FINAL) == Cst(etype(STATE,0,4));Type(mbc_board_id) == Cst(btype(INTEGER,?,?));Type(OP_IMove_moveForward) == Cst(btype(INTEGER,?,?));Type(OP_IMove_turn) == Cst(btype(INTEGER,?,?));Type(OP_IMove_avoid_enableCollisionDetection_disableCollisionDetection) == Cst(btype(INTEGER,?,?));Type(max_board_output) == Cst(btype(INTEGER,?,?));Type(SimSquare_cycle_def) == Cst(btype(INTEGER,?,?));Type(linear) == Cst(btype(INTEGER,?,?));Type(angular) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
