...
typedef struct _struct_constants {
	...
	uint8_t co_mbc_board_id;
	uint8_t co_OP_IMove_moveForward;
	uint8_t co_OP_IMove_turn;
	uint8_t co_OP_IMove_avoid_enableCollisionDetection_disableCollisionDetection;
	uint8_t co_max_board_output;
	uint32_t co_SimSquare_cycle_def;
	uint32_t co_linear;
	uint32_t co_angular;
} struct_constants;
