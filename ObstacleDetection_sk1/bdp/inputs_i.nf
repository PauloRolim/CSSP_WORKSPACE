...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(inputs_i),read_inputs)==(btrue & 0: uint8_t & 0<MAX_NB_INPUTS & i_ObstacleI_obstacle: uint8_t | @(state$1).(state$1: uint8_t & (state$1 = IO_ON or state$1 = IO_OFF) ==> i_ObstacleI_obstacle:=state$1));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(inputs_i)) == (? | ? | ? | ? | read_inputs,get_i_ObstacleI_obstacle | ? | seen(Machine(g_types)),seen(Machine(io_constants)),seen(Machine(lchip_configuration)),seen(Machine(lchip_interface)) | ? | inputs_i);
