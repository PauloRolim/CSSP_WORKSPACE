...
&
THEORY ListSeesX IS
  List_Sees(Implementation(logic_i))==(g_types,g_operators,io_constants,lchip_interface,user_ctx)
END
&
...
THEORY ListInvariantX IS
  ...
  Context_List_Invariant(Implementation(logic_i))==(ms_tick: uint32_t);
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(logic_i)) == (? | ? | ? | ? | user_logic,get_board_0_O1,get_board_0_O2 | ? | seen(Machine(g_types)),seen(Machine(g_operators)),seen(Machine(io_constants)),seen(Machine(lchip_interface)),seen(Machine(user_ctx)) | ? | logic_i);
