...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(user_ctx))==(delta_t,delta_t1);
  Inherited_List_Constants(Machine(user_ctx))==(?);
  List_Constants(Machine(user_ctx))==(delta_t,delta_t1)
END
&
THEORY ListSetsX IS
...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(user_ctx))==(delta_t: uint32_t & not(delta_t = 0) & delta_t1: uint32_t & not(delta_t1 = 0))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(user_ctx)) == (Type(delta_t) == Cst(btype(INTEGER,?,?));Type(delta_t1) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
