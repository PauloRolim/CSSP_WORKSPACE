...
THEORY ListConstantsX IS
  List_Valuable_Constants(Implementation(user_ctx_i))==(delta_t,delta_t1);
  Inherited_List_Constants(Implementation(user_ctx_i))==(delta_t,delta_t1);
...
THEORY ListPropertiesX IS
  Abstract_List_Properties(Implementation(user_ctx_i))==(delta_t: uint32_t & not(delta_t = 0) & delta_t1: uint32_t & not(delta_t1 = 0));
...
THEORY ListValuesX IS
  ...
  Values_Subs(Implementation(user_ctx_i))==(delta_t,delta_t1: 1000,2500);
  List_Values(Implementation(user_ctx_i))==(delta_t = 1000;delta_t1 = 2500)
END
&
...
THEORY InheritedEnvX IS
  Constants(Implementation(user_ctx_i))==(Type(delta_t) == Cst(btype(INTEGER,?,?));Type(delta_t1) == Cst(btype(INTEGER,?,?)))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Implementation(user_ctx_i)) == (Type(delta_t1) == Cst(btype(INTEGER,?,?));Type(delta_t) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
