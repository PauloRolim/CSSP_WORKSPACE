...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(logic_i),user_logic)==(btrue | @(local_input01,local_input02).(@(local_input01$2).(local_input01$2: uint8_t ==> local_input01:=local_input01$2);@(local_input02$2).(local_input02$2: uint8_t ==> local_input02:=local_input02$2);(local_input01: uint8_t | local_input01:=board_0_I1);(local_input02: uint8_t | local_input02:=board_0_I2);(local_input01 = IO_ON ==> (local_input02 = IO_ON ==> (IO_OFF: INT | board_0_O1:=IO_OFF) [] not(local_input02 = IO_ON) ==> (IO_ON: INT | board_0_O1:=IO_ON)) [] not(local_input01 = IO_ON) ==> (IO_ON: INT | board_0_O1:=IO_ON))));
  List_Substitution(Implementation(logic_i),user_logic)==(VAR local_input01,local_input02 IN local_input01: (local_input01: uint8_t);local_input02: (local_input02: uint8_t);local_input01 <-- get_board_0_I1;local_input02 <-- get_board_0_I2;IF local_input01 = IO_ON THEN IF local_input02 = IO_ON THEN board_0_O1:=IO_OFF ELSE board_0_O1:=IO_ON END ELSE board_0_O1:=IO_ON END END);
...
THEORY VariablesLocEnvX IS
  Variables_Loc(Implementation(logic_i),user_logic, 1) == (Type(local_input01) == Lvl(btype(INTEGER,?,?));Type(local_input02) == Lvl(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
