...
THEORY ListVariablesX IS
  ...
  Abstract_List_Variables(Implementation(logic_i))==(O2,O1);
...
THEORY ListVisibleVariablesX IS
  ...
  List_VisibleVariables(Implementation(logic_i))==(O2,O1);
  Internal_List_VisibleVariables(Implementation(logic_i))==(O2,O1)
END
&
THEORY ListInvariantX IS
  Gluing_Seen_List_Invariant(Implementation(logic_i))==(btrue);
  Expanded_List_Invariant(Implementation(logic_i))==(btrue);
  Abstract_List_Invariant(Implementation(logic_i))==(O1: uint8_t & O2: uint8_t);
  Context_List_Invariant(Implementation(logic_i))==(I1: uint8_t & I2: uint8_t & I3: uint8_t);
  List_Invariant(Implementation(logic_i))==(O1: uint8_t & O2: uint8_t)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Implementation(logic_i))==((IO_OFF: INT | O1:=IO_OFF);(IO_OFF: INT | O2:=IO_OFF));
  Context_List_Initialisation(Implementation(logic_i))==(skip);
  List_Initialisation(Implementation(logic_i))==(O1:=IO_OFF;O2:=IO_OFF)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Implementation(logic_i))==(triAND,negIO,user_logic,get_O1,get_O2);
  List_Operations(Implementation(logic_i))==(triAND,negIO,user_logic,get_O1,get_O2)
END
&
THEORY ListInputX IS
  List_Input(Implementation(logic_i),triAND)==(v1,v2,v3);
  List_Input(Implementation(logic_i),negIO)==(val);
...
THEORY ListOutputX IS
  List_Output(Implementation(logic_i),triAND)==(res);
  List_Output(Implementation(logic_i),negIO)==(res);
...
THEORY ListHeaderX IS
  List_Header(Implementation(logic_i),triAND)==(res <-- triAND(v1,v2,v3));
  List_Header(Implementation(logic_i),negIO)==(res <-- negIO(val));
...
THEORY ListPreconditionX IS
  Own_Precondition(Implementation(logic_i),triAND)==(btrue);
  List_Precondition(Implementation(logic_i),triAND)==(res: uint8_t & v1: uint8_t & v2: uint8_t & v3: uint8_t);
  Own_Precondition(Implementation(logic_i),negIO)==(btrue);
  List_Precondition(Implementation(logic_i),negIO)==(res: uint8_t & val: uint8_t);
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(logic_i),user_logic)==(btrue | @(i1_,i2_,i3_).(@(i1_$2).(i1_$2: uint8_t ==> i1_:=i1_$2);@(i2_$2).(i2_$2: uint8_t ==> i2_:=i2_$2);@(i3_$2).(i3_$2: uint8_t ==> i3_:=i3_$2);(i1_: uint8_t | i1_:=I1);(i2_: uint8_t | i2_:=I2);(i3_: uint8_t | i3_:=I3);(IO_OFF: INT | O1:=IO_OFF);(i1_ = IO_ON ==> (i2_ = IO_ON ==> (i3_ = IO_ON ==> (IO_ON: INT | O1:=IO_ON) [] not(i3_ = IO_ON) ==> skip) [] not(i2_ = IO_ON) ==> skip) [] not(i1_ = IO_ON) ==> skip);(O1 = IO_ON ==> (IO_OFF: INT | O2:=IO_OFF) [] not(O1 = IO_ON) ==> (IO_ON: INT | O2:=IO_ON))));
  Expanded_List_Substitution(Implementation(logic_i),negIO)==(res: uint8_t & val: uint8_t | val = IO_ON ==> (IO_OFF: INT | res:=IO_OFF) [] not(val = IO_ON) ==> (IO_ON: INT | res:=IO_ON));
  Expanded_List_Substitution(Implementation(logic_i),triAND)==(res: uint8_t & v1: uint8_t & v2: uint8_t & v3: uint8_t | (IO_OFF: INT | res:=IO_OFF);(v1 = IO_ON ==> (v2 = IO_ON ==> (v3 = IO_ON ==> (IO_ON: INT | res:=IO_ON) [] not(v3 = IO_ON) ==> skip) [] not(v2 = IO_ON) ==> skip) [] not(v1 = IO_ON) ==> skip));
  List_Substitution(Implementation(logic_i),triAND)==(res:=IO_OFF;IF v1 = IO_ON THEN IF v2 = IO_ON THEN IF v3 = IO_ON THEN res:=IO_ON END END END);
  List_Substitution(Implementation(logic_i),negIO)==(IF val = IO_ON THEN res:=IO_OFF ELSE res:=IO_ON END);
  List_Substitution(Implementation(logic_i),user_logic)==(VAR i1_,i2_,i3_ IN i1_: (i1_: uint8_t);i2_: (i2_: uint8_t);i3_: (i3_: uint8_t);i1_ <-- get_I1;i2_ <-- get_I2;i3_ <-- get_I3;O1:=IO_OFF;IF i1_ = IO_ON THEN IF i2_ = IO_ON THEN IF i3_ = IO_ON THEN O1:=IO_ON END END END;IF O1 = IO_ON THEN O2:=IO_OFF ELSE O2:=IO_ON END END);
...
THEORY ListSeenInfoX IS
  Seen_Internal_List_Operations(Implementation(logic_i),Machine(inputs))==(read_inputs,get_I1,get_I2,get_I3);
  ...
  Seen_Expanded_List_Substitution(Implementation(logic_i),read_inputs)==(@(I1$0).(I1$0: uint8_t ==> I1:=I1$0) || @(I2$0).(I2$0: uint8_t ==> I2:=I2$0) || @(I3$0).(I3$0: uint8_t ==> I3:=I3$0));
  Seen_List_Operations(Implementation(logic_i),Machine(inputs))==(read_inputs,get_I1,get_I2,get_I3);
  ...
  Seen_Expanded_List_Substitution(Implementation(logic_i),get_ms_tick)==(@(out$0).(out$0: uint32_t ==> out:=out$0));
  ...
  Seen_List_Precondition(Implementation(logic_i),write_global_output)==(output_id: uint8_t & state: uint8_t);
  Seen_Expanded_List_Substitution(Implementation(logic_i),write_global_output)==(skip);
  Seen_List_Precondition(Implementation(logic_i),read_global_input)==(input_id: uint8_t & state: uint8_t);
...
THEORY ListIncludedOperationsX IS
  List_Included_Operations(Implementation(logic_i),Machine(inputs))==(read_inputs,get_I1,get_I2,get_I3);
...
THEORY InheritedEnvX IS
  VisibleVariables(Implementation(logic_i))==(Type(O1) == Mvv(btype(INTEGER,?,?));Type(O2) == Mvv(btype(INTEGER,?,?)));
  Operations(Implementation(logic_i))==(Type(get_O2) == Cst(btype(INTEGER,?,?),No_type);Type(get_O1) == Cst(btype(INTEGER,?,?),No_type);Type(user_logic) == Cst(No_type,No_type))
END
&
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(logic_i)) == (? | ? | ? | ? | triAND,negIO,user_logic,get_O1,get_O2,refinement_of_negIO,refinement_of_triAND | ? | seen(Machine(g_types)),seen(Machine(g_operators)),seen(Machine(io_constants)),seen(Machine(lchip_interface)),seen(Machine(user_ctx)),seen(Machine(inputs)) | ? | logic_i);
  List_Of_HiddenCst_Ids(Implementation(logic_i)) == (? | ?);
  List_Of_VisibleCst_Ids(Implementation(logic_i)) == (?);
  List_Of_VisibleVar_Ids(Implementation(logic_i)) == (O2,O1 | ?);
...
THEORY VisibleVariablesEnvX IS
  VisibleVariables(Implementation(logic_i)) == (Type(O2) == Mvv(btype(INTEGER,?,?));Type(O1) == Mvv(btype(INTEGER,?,?)))
END
&
...
THEORY ListLocalOperationsX IS
  List_Local_Operations(Implementation(logic_i))==(triAND,negIO)
END
&
THEORY ListLocalInputX IS
  List_Local_Input(Implementation(logic_i),triAND)==(v1,v2,v3);
  List_Local_Input(Implementation(logic_i),negIO)==(val)
END
&
THEORY ListLocalOutputX IS
  List_Local_Output(Implementation(logic_i),triAND)==(res);
  List_Local_Output(Implementation(logic_i),negIO)==(res)
END
&
THEORY ListLocalHeaderX IS
  List_Local_Header(Implementation(logic_i),triAND)==(res <-- triAND(v1,v2,v3));
  List_Local_Header(Implementation(logic_i),negIO)==(res <-- negIO(val))
END
&
THEORY ListLocalPreconditionX IS
  List_Local_Precondition(Implementation(logic_i),triAND)==(res: uint8_t & v1: uint8_t & v2: uint8_t & v3: uint8_t);
  List_Local_Precondition(Implementation(logic_i),negIO)==(res: uint8_t & val: uint8_t)
END
&
THEORY ListLocalSubstitutionX IS
  Expanded_List_Local_Substitution(Implementation(logic_i),negIO)==(res: uint8_t & val: uint8_t | @(res$0).(res$0: uint8_t ==> res:=res$0));
  Expanded_List_Local_Substitution(Implementation(logic_i),triAND)==(res: uint8_t & v1: uint8_t & v2: uint8_t & v3: uint8_t | @(res$0).(res$0: uint8_t ==> res:=res$0));
  List_Local_Substitution(Implementation(logic_i),triAND)==(res:: uint8_t);
  List_Local_Substitution(Implementation(logic_i),negIO)==(res:: uint8_t)
END
&
THEORY TypingPredicateX IS
  TypingPredicate(Implementation(logic_i))==(O1: INTEGER & O2: INTEGER)
END
&
THEORY ImportedVariablesListX END
&
THEORY ListLocalOpInvariantX IS
  LocalOp_Expanded_Invariant(Implementation(logic_i),negIO)==(btrue);
  LocalOp_Expanded_Invariant(Implementation(logic_i),triAND)==(btrue)
END
)
