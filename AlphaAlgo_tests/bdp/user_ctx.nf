...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(user_ctx))==(cycle_unit: uint32_t & SimSMovement_cycle_def: uint32_t & av: uint32_t & lv: uint32_t & pi: uint32_t & zero: uint32_t & av: -3..3 & lv: 0..3 & alpha: uint8_t & STATE: FIN(INTEGER) & not(STATE = {}))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(user_ctx)) == (Type(INIT) == Cst(etype(STATE,0,2));Type(EXEC_1) == Cst(etype(STATE,0,2));Type(EXEC_2) == Cst(etype(STATE,0,2));Type(cycle_unit) == Cst(btype(INTEGER,?,?));Type(SimSMovement_cycle_def) == Cst(btype(INTEGER,?,?));Type(av) == Cst(btype(INTEGER,?,?));Type(lv) == Cst(btype(INTEGER,?,?));Type(pi) == Cst(btype(INTEGER,?,?));Type(zero) == Cst(btype(INTEGER,?,?));Type(alpha) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
