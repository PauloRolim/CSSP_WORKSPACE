...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(user_ctx))==(DELTA_T);
  Inherited_List_Constants(Machine(user_ctx))==(?);
  List_Constants(Machine(user_ctx))==(DELTA_T)
END
&
THEORY ListSetsX IS
...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(user_ctx))==(DELTA_T: uint32_t)
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(user_ctx)) == (Type(DELTA_T) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
