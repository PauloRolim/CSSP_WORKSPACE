...
THEORY ListVisibleVariablesX IS
  ...
  List_VisibleVariables(Implementation(logic_i))==(STATUS,TIME_A,O2,O1);
  Internal_List_VisibleVariables(Implementation(logic_i))==(STATUS,TIME_A,O2,O1)
END
&
THEORY ListInvariantX IS
  ...
  List_Invariant(Implementation(logic_i))==(O1: uint8_t & O2: uint8_t & TIME_A: uint32_t & STATUS: uint8_t)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Implementation(logic_i))==((IO_OFF: INT | O1:=IO_OFF);(IO_OFF: INT | O2:=IO_OFF);(0: INT | TIME_A:=0);(SFALSE: INT | STATUS:=SFALSE));
  Context_List_Initialisation(Implementation(logic_i))==(skip);
  List_Initialisation(Implementation(logic_i))==(O1:=IO_OFF;O2:=IO_OFF;TIME_A:=0;STATUS:=SFALSE)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Implementation(logic_i))==(user_logic,get_O1,get_O2);
  List_Operations(Implementation(logic_i))==(user_logic,get_O1,get_O2)
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(logic_i),user_logic)==(btrue | @(ltime_,deltat_,ms_tick_cycle,s_tick_cycle,aa).(@(ltime_$2).(ltime_$2: uint32_t ==> ltime_:=ltime_$2);@(deltat_$2).(deltat_$2: uint32_t ==> deltat_:=deltat_$2);(ltime_: uint32_t | @(out$0).(out$0: uint32_t ==> ltime_:=out$0));(sub_uint32(ltime_,TIME_A): INT & ltime_,TIME_A: dom(sub_uint32) | deltat_:=sub_uint32(ltime_,TIME_A));@(ms_tick_cycle$2).(ms_tick_cycle$2: uint32_t ==> ms_tick_cycle:=ms_tick_cycle$2);@(s_tick_cycle$2).(s_tick_cycle$2: uint32_t ==> s_tick_cycle:=s_tick_cycle$2);@(aa$2).(aa$2: uint32_t ==> aa:=aa$2);(ms_tick_cycle: uint32_t | @(out$0).(out$0: uint32_t ==> ms_tick_cycle:=out$0));(ms_tick_cycle/1000: INT & ms_tick_cycle: INT & 1000: INT & not(1000 = 0) | s_tick_cycle:=ms_tick_cycle/1000);(s_tick_cycle mod 2: INT & s_tick_cycle mod 2: NAT & s_tick_cycle: NAT & 2: NAT1 | aa:=s_tick_cycle mod 2);(IO_OFF: INT | O2:=IO_OFF);(aa = 0 ==> (IO_ON: INT | O2:=IO_ON) [] not(aa = 0) ==> skip);(DELTA_T<=deltat_ ==> (STATUS = STRUE ==> (SFALSE: INT | STATUS:=SFALSE) [] not(STATUS = STRUE) ==> (STRUE: INT | STATUS:=STRUE);(ltime_: INT | TIME_A:=ltime_)) [] not(DELTA_T<=deltat_) ==> skip);(STATUS = STRUE ==> (IO_ON: INT | O1:=IO_ON) [] not(STATUS = STRUE) ==> (IO_OFF: INT | O1:=IO_OFF));(IO_OFF: INT | O1:=IO_OFF)));
  List_Substitution(Implementation(logic_i),user_logic)==(VAR ltime_,deltat_,ms_tick_cycle,s_tick_cycle,aa IN ltime_: (ltime_: uint32_t);deltat_: (deltat_: uint32_t);ltime_ <-- get_ms_tick;deltat_:=sub_uint32(ltime_,TIME_A);ms_tick_cycle: (ms_tick_cycle: uint32_t);s_tick_cycle: (s_tick_cycle: uint32_t);aa: (aa: uint32_t);ms_tick_cycle <-- get_ms_tick;s_tick_cycle:=ms_tick_cycle/1000;aa:=s_tick_cycle mod 2;O2:=IO_OFF;IF aa = 0 THEN O2:=IO_ON END;IF DELTA_T<=deltat_ THEN IF STATUS = STRUE THEN STATUS:=SFALSE ELSE STATUS:=STRUE END;TIME_A:=ltime_ END;IF STATUS = STRUE THEN O1:=IO_ON ELSE O1:=IO_OFF END;O1:=IO_OFF END);
...
THEORY InheritedEnvX IS
  VisibleVariables(Implementation(logic_i))==(Type(O1) == Mvv(btype(INTEGER,?,?));Type(O2) == Mvv(btype(INTEGER,?,?));Type(TIME_A) == Mvv(btype(INTEGER,?,?));Type(STATUS) == Mvv(btype(INTEGER,?,?)));
...
THEORY ListVisibleStaticX IS
  List_Constants(Implementation(logic_i),Machine(user_ctx))==(DELTA_T);
  List_Constants_Env(Implementation(logic_i),Machine(user_ctx))==(Type(DELTA_T) == Cst(btype(INTEGER,?,?)));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(logic_i)) == (? | ? | ? | ? | user_logic,get_O1,get_O2 | ? | seen(Machine(g_types)),seen(Machine(g_operators)),seen(Machine(io_constants)),seen(Machine(lchip_interface)),seen(Machine(user_ctx)),seen(Machine(inputs)) | ? | logic_i);
  List_Of_HiddenCst_Ids(Implementation(logic_i)) == (? | ?);
  List_Of_VisibleCst_Ids(Implementation(logic_i)) == (?);
  List_Of_VisibleVar_Ids(Implementation(logic_i)) == (STATUS,TIME_A,O2,O1 | ?);
...
THEORY VisibleVariablesEnvX IS
  VisibleVariables(Implementation(logic_i)) == (Type(STATUS) == Mvv(btype(INTEGER,?,?));Type(TIME_A) == Mvv(btype(INTEGER,?,?));Type(O2) == Mvv(btype(INTEGER,?,?));Type(O1) == Mvv(btype(INTEGER,?,?)))
END
&
THEORY VariablesLocEnvX IS
  Variables_Loc(Implementation(logic_i),user_logic, 1) == (Type(ltime_) == Lvl(btype(INTEGER,?,?));Type(deltat_) == Lvl(btype(INTEGER,?,?));Type(ms_tick_cycle) == Lvl(btype(INTEGER,?,?));Type(s_tick_cycle) == Lvl(btype(INTEGER,?,?));Type(aa) == Lvl(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
...
THEORY TypingPredicateX IS
  TypingPredicate(Implementation(logic_i))==(O1: INTEGER & O2: INTEGER & TIME_A: INTEGER & STATUS: INTEGER)
END
&
