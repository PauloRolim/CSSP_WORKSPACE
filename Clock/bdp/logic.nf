...
THEORY ListVariablesX IS
  ...
  Local_List_Variables(Machine(logic))==(O2,O1);
  List_Variables(Machine(logic))==(O2,O1);
  External_List_Variables(Machine(logic))==(O2,O1)
END
&
...
THEORY ListInvariantX IS
  ...
  List_Invariant(Machine(logic))==(O1: uint8_t & O2: uint8_t)
END
&
...
THEORY ListInitialisationX IS
  Expanded_List_Initialisation(Machine(logic))==(@(O1$0).(O1$0: uint8_t ==> O1:=O1$0) || @(O2$0).(O2$0: uint8_t ==> O2:=O2$0));
  Context_List_Initialisation(Machine(logic))==(skip);
  List_Initialisation(Machine(logic))==(O1:: uint8_t || O2:: uint8_t)
END
&
...
THEORY ListOperationsX IS
  Internal_List_Operations(Machine(logic))==(user_logic,get_O1,get_O2);
  List_Operations(Machine(logic))==(user_logic,get_O1,get_O2)
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Machine(logic),user_logic)==(btrue | @(O1$0).(O1$0: uint8_t ==> O1:=O1$0) || @(O2$0).(O2$0: uint8_t ==> O2:=O2$0));
  List_Substitution(Machine(logic),user_logic)==(O1:: uint8_t || O2:: uint8_t);
...
THEORY ListSeenInfoX IS
  ...
  Seen_Expanded_List_Substitution(Machine(logic),get_ms_tick)==(@(out$0).(out$0: uint32_t ==> out:=out$0));
  ...
  Seen_List_Precondition(Machine(logic),write_global_output)==(output_id: uint8_t & state: uint8_t);
  Seen_Expanded_List_Substitution(Machine(logic),write_global_output)==(skip);
  Seen_List_Precondition(Machine(logic),read_global_input)==(input_id: uint8_t & state: uint8_t);
...
THEORY VariablesEnvX IS
  Variables(Machine(logic)) == (Type(O2) == Mvl(btype(INTEGER,?,?));Type(O1) == Mvl(btype(INTEGER,?,?)))
END
&
THEORY OperationsEnvX IS
  Operations(Machine(logic)) == (Type(get_O2) == Cst(btype(INTEGER,?,?),No_type);Type(get_O1) == Cst(btype(INTEGER,?,?),No_type);Type(user_logic) == Cst(No_type,No_type));
  Observers(Machine(logic)) == (Type(get_O2) == Cst(btype(INTEGER,?,?),No_type);Type(get_O1) == Cst(btype(INTEGER,?,?),No_type))
END
&
THEORY TCIntRdX IS
