...
THEORY ListConstantsX IS
  List_Valuable_Constants(Machine(user_ctx))==(linear,angular);
  Inherited_List_Constants(Machine(user_ctx))==(?);
  List_Constants(Machine(user_ctx))==(linear,angular)
END
&
THEORY ListSetsX IS
  Set_Definition(Machine(user_ctx),STATE)==({NONE,SMovingForward,SObserving,DObserving,STurning,DTurning,Waiting,EObserving,SCollision});
...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(user_ctx))==(linear: uint8_t & angular: uint8_t & STATE: FIN(INTEGER) & not(STATE = {}))
END
&
