...
THEORY ListOperationsX IS
  Internal_List_Operations(Implementation(logic_i))==(moveFoward,turn,avoid,enableCollisionDetection,disableCollisionDetection,since,read_master_clock,land,lor,lnot,ldiff,user_logic,get_board_0_O1,get_board_0_O2,get_board_0_O3,get_board_0_O4,get_board_0_O5,get_board_0_O6,get_board_0_O7,get_board_0_O8);
  List_Operations(Implementation(logic_i))==(moveFoward,turn,avoid,enableCollisionDetection,disableCollisionDetection,since,read_master_clock,land,lor,lnot,ldiff,user_logic,get_board_0_O1,get_board_0_O2,get_board_0_O3,get_board_0_O4,get_board_0_O5,get_board_0_O6,get_board_0_O7,get_board_0_O8)
END
&
...
THEORY ListSubstitutionX IS
  ...
  Expanded_List_Substitution(Implementation(logic_i),user_logic)==(btrue | @(local_collisionDetected,C_5,Segment4).(@(local_collisionDetected$2).(local_collisionDetected$2: uint8_t ==> local_collisionDetected:=local_collisionDetected$2);@(C_5$2).(C_5$2: BOOL ==> C_5:=C_5$2);@(Segment4$2).(Segment4$2: BOOL ==> Segment4:=Segment4$2);(local_collisionDetected: uint8_t | local_collisionDetected:=board_0_I1);(C_: uint8_t & C_5: BOOL & 5: uint8_t | @(result$0).(result$0: BOOL ==> C_5:=result$0));(segment: uint8_t & Segment4: BOOL & 4: uint8_t | @(result$0).(result$0: BOOL ==> Segment4:=result$0));(decision = NONE ==> decision:=SMovingForward [] not(decision = NONE) ==> skip);(decision = SMovingForward ==> ((linear: uint8_t | skip);decision:=SObserving) [] not(decision = SMovingForward) ==> skip);(decision = SObserving ==> ((btrue | skip);decision:=DObserving) [] not(decision = SObserving) ==> skip);(decision = DObserving ==> (local_collisionDetected = IO_OFF ==> (3<=C_ ==> (C_5 = TRUE ==> (4<=segment ==> (C_5 = TRUE ==> (Segment4 = TRUE ==> decision:=DObserving [] not(Segment4 = TRUE) ==> skip) [] not(C_5 = TRUE) ==> skip) [] not(4<=segment) ==> skip) [] not(C_5 = TRUE) ==> skip) [] not(3<=C_) ==> skip) [] not(local_collisionDetected = IO_OFF) ==> skip;(C_5 = TRUE ==> (segment<4 ==> ((btrue | skip);(add_uint8(segment,1): INT & segment,1: dom(add_uint8) | segment:=add_uint8(segment,1));decision:=STurning) [] not(segment<4) ==> skip) [] not(C_5 = TRUE) ==> skip);(local_collisionDetected = IO_ON ==> (3<=C_ ==> decision:=EObserving [] not(3<=C_) ==> skip) [] not(local_collisionDetected = IO_ON) ==> skip);(decision = EObserving ==> ((btrue | skip);decision:=SCollision) [] not(decision = EObserving) ==> skip);(decision = SCollision ==> ((btrue | skip);(0: INT | D_:=0);decision:=Waiting) [] not(decision = SCollision) ==> skip);(decision = Waiting ==> skip [] not(decision = Waiting) ==> skip)) [] not(decision = DObserving) ==> skip);(decision = STurning ==> ((IO_ON: INT | board_0_O8:=IO_ON);(angular: uint8_t | skip);decision:=DTurning;(current_time: INT | clock_memo:=current_time)) [] not(decision = STurning) ==> skip);(decision = DTurning ==> @time_elapsed.(@(time_elapsed$2).(time_elapsed$2: uint32_t ==> time_elapsed:=time_elapsed$2);(sub_uint32(current_time,clock_memo): INT & current_time,clock_memo: dom(sub_uint32) | time_elapsed:=sub_uint32(current_time,clock_memo));(time_elapsed<2 ==> decision:=DTurning [] not(time_elapsed<2) ==> (time_elapsed = 2 ==> ((0: INT | C_:=0);decision:=SMovingForward) [] not(time_elapsed = 2) ==> skip))) [] not(decision = DTurning) ==> skip)));
  ...
  List_Substitution(Implementation(logic_i),user_logic)==(VAR local_collisionDetected,C_5,Segment4 IN local_collisionDetected: (local_collisionDetected: uint8_t);C_5: (C_5: BOOL);Segment4: (Segment4: BOOL);local_collisionDetected <-- get_board_0_I1;C_5 <-- ldiff(C_,5);Segment4 <-- ldiff(segment,4);IF decision = NONE THEN decision:=SMovingForward END;IF decision = SMovingForward THEN moveFoward(linear);decision:=SObserving END;IF decision = SObserving THEN enableCollisionDetection;decision:=DObserving END;IF decision = DObserving THEN IF local_collisionDetected = IO_OFF THEN IF 3<=C_ THEN IF C_5 = TRUE THEN IF 4<=segment THEN IF C_5 = TRUE THEN IF Segment4 = TRUE THEN decision:=DObserving END END END END END END;IF C_5 = TRUE THEN IF segment<4 THEN disableCollisionDetection;segment:=add_uint8(segment,1);decision:=STurning END END;IF local_collisionDetected = IO_ON THEN IF 3<=C_ THEN decision:=EObserving END END;IF decision = EObserving THEN disableCollisionDetection;decision:=SCollision END;IF decision = SCollision THEN avoid;D_:=0;decision:=Waiting END;IF decision = Waiting THEN skip END END;IF decision = STurning THEN board_0_O8:=IO_ON;turn(angular);decision:=DTurning;clock_memo:=current_time END;IF decision = DTurning THEN VAR time_elapsed IN time_elapsed: (time_elapsed: uint32_t);time_elapsed:=sub_uint32(current_time,clock_memo);IF time_elapsed<2 THEN decision:=DTurning ELSE IF time_elapsed = 2 THEN C_:=0;decision:=SMovingForward END END END END END);
...
&
THEORY ListSetsX IS
  Set_Definition(Implementation(logic_i),STATE)==({NONE,SMovingForward,SObserving,DObserving,STurning,DTurning,Waiting,EObserving,SCollision});
...
THEORY ListVisibleStaticX IS
  ...
  List_Constants_Env(Implementation(logic_i),Machine(user_ctx))==(Type(NONE) == Cst(etype(STATE,0,8));Type(SMovingForward) == Cst(etype(STATE,0,8));Type(SObserving) == Cst(etype(STATE,0,8));Type(DObserving) == Cst(etype(STATE,0,8));Type(STurning) == Cst(etype(STATE,0,8));Type(DTurning) == Cst(etype(STATE,0,8));Type(Waiting) == Cst(etype(STATE,0,8));Type(EObserving) == Cst(etype(STATE,0,8));Type(SCollision) == Cst(etype(STATE,0,8));Type(linear) == Cst(btype(INTEGER,?,?));Type(angular) == Cst(btype(INTEGER,?,?)));
  Enumerate_Definition(Implementation(logic_i),Machine(user_ctx),STATE)==({NONE,SMovingForward,SObserving,DObserving,STurning,DTurning,Waiting,EObserving,SCollision});
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(logic_i)) == (? | ? | ? | ? | moveFoward,turn,avoid,enableCollisionDetection,disableCollisionDetection,since,read_master_clock,land,lor,lnot,ldiff,user_logic,get_board_0_O1,get_board_0_O2,get_board_0_O3,get_board_0_O4,get_board_0_O5,get_board_0_O6,get_board_0_O7,get_board_0_O8,refinement_of_ldiff,refinement_of_lnot,refinement_of_lor,refinement_of_land,refinement_of_read_master_clock,refinement_of_since,refinement_of_disableCollisionDetection,refinement_of_enableCollisionDetection,refinement_of_avoid,refinement_of_turn,refinement_of_moveFoward | ? | seen(Machine(g_types)),seen(Machine(g_operators)),seen(Machine(io_constants)),seen(Machine(lchip_interface)),seen(Machine(user_ctx)),seen(Machine(inputs)) | ? | logic_i);
  ...
  List_Of_Ids(Machine(user_ctx)) == (linear,angular,STATE,NONE,SMovingForward,SObserving,DObserving,STurning,DTurning,Waiting,EObserving,SCollision | ? | ? | ? | ? | ? | seen(Machine(g_types)) | ? | user_ctx);
...
THEORY VariablesLocEnvX IS
  ...
  Variables_Loc(Implementation(logic_i),user_logic, 2) == (Type(time_elapsed) == Lvl(btype(INTEGER,?,?)));
  Variables_Loc(Implementation(logic_i),user_logic, 1) == (Type(local_collisionDetected) == Lvl(btype(INTEGER,?,?));Type(C_5) == Lvl(btype(BOOL,?,?));Type(Segment4) == Lvl(btype(BOOL,?,?)))
END
&
THEORY TCIntRdX IS
...
THEORY ListLocalOperationsX IS
  List_Local_Operations(Implementation(logic_i))==(moveFoward,turn,avoid,enableCollisionDetection,disableCollisionDetection,since,read_master_clock,land,lor,lnot,ldiff)
END
&
