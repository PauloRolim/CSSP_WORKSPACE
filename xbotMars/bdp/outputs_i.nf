...
THEORY ListInvariantX IS
  ...
  Context_List_Invariant(Implementation(outputs_i))==(ms_tick: uint32_t & LinearVel: uint8_t & AngularVel: uint8_t & COUNTER: uint8_t);
...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Implementation(outputs_i),write_outputs)==(btrue | @lsb.(@(lsb$2).(lsb$2: uint8_t ==> lsb:=lsb$2);(lsb: uint8_t | lsb:=LinearVel);(0: uint8_t & 0<MAX_NB_OUTPUTS & lsb: uint8_t | skip);(lsb: uint8_t | lsb:=AngularVel);(1: uint8_t & 1<MAX_NB_OUTPUTS & lsb: uint8_t | skip)));
  List_Substitution(Implementation(outputs_i),write_outputs)==(VAR lsb IN lsb: (lsb: uint8_t);lsb <-- get_LinearVel;write_global_output(0,lsb);lsb <-- get_AngularVel;write_global_output(1,lsb) END)
END
&
...
THEORY ListSeenInfoX IS
  Seen_Internal_List_Operations(Implementation(outputs_i),Machine(logic))==(user_logic,get_LinearVel,get_AngularVel);
  ...
  Seen_List_Precondition(Implementation(outputs_i),get_AngularVel)==(po: uint8_t);
  Seen_Expanded_List_Substitution(Implementation(outputs_i),get_AngularVel)==(po:=AngularVel);
  Seen_List_Precondition(Implementation(outputs_i),get_LinearVel)==(po: uint8_t);
  Seen_Expanded_List_Substitution(Implementation(outputs_i),get_LinearVel)==(po:=LinearVel);
  Seen_List_Precondition(Implementation(outputs_i),user_logic)==(btrue);
  Seen_Expanded_List_Substitution(Implementation(outputs_i),user_logic)==(@(LinearVel$1,AngularVel$1,COUNTER$1).(LinearVel$1: uint8_t & AngularVel$1: uint8_t & COUNTER$1: uint8_t & (LinearVel$1 = IO_ON & AngularVel$1 = IO_OFF or (LinearVel$1 = IO_OFF & AngularVel$1 = IO_ON)) & COUNTER$1 = 0 <=> (LinearVel$1 = IO_OFF & AngularVel$1 = IO_ON) & COUNTER$1 = 1 <=> (LinearVel$1 = IO_ON & AngularVel$1 = IO_OFF) ==> LinearVel,AngularVel,COUNTER:=LinearVel$1,AngularVel$1,COUNTER$1));
  Seen_List_Operations(Implementation(outputs_i),Machine(logic))==(user_logic,get_LinearVel,get_AngularVel);
...
THEORY ListIncludedOperationsX IS
  List_Included_Operations(Implementation(outputs_i),Machine(logic))==(user_logic,get_LinearVel,get_AngularVel);
