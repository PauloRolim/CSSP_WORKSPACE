...
THEORY ListPropertiesX IS
  ...
  List_Properties(Machine(user_ctx))==(cycle_unit: uint32_t & SimSMovement_cycle_def: uint32_t & av: uint32_t & lv: uint32_t & pi: uint32_t & av: 0..3 & lv: 0..3)
END
&
...
THEORY ConstantsEnvX IS
  Constants(Machine(user_ctx)) == (Type(cycle_unit) == Cst(btype(INTEGER,?,?));Type(SimSMovement_cycle_def) == Cst(btype(INTEGER,?,?));Type(av) == Cst(btype(INTEGER,?,?));Type(lv) == Cst(btype(INTEGER,?,?));Type(pi) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
