...
THEORY ListPropertiesX IS
  ...
  Inherited_List_Properties(Implementation(user_component_i))==(cycle_unit: uint32_t & SimSMovement_cycle_def: uint32_t & av: uint32_t & lv: uint32_t & pi: uint32_t & av: 0..3 & lv: 0..3);
...
THEORY ListVisibleStaticX IS
  ...
  List_Constants_Env(Implementation(user_component_i),Machine(user_ctx))==(Type(cycle_unit) == Cst(btype(INTEGER,?,?));Type(SimSMovement_cycle_def) == Cst(btype(INTEGER,?,?));Type(av) == Cst(btype(INTEGER,?,?));Type(lv) == Cst(btype(INTEGER,?,?));Type(pi) == Cst(btype(INTEGER,?,?)));
...
THEORY ListOfIdsX IS
  List_Of_Ids(Implementation(user_component_i)) == (? | cycle_unit,SimSMovement_cycle_def,av,lv,pi | ? | board_0_O2,board_0_O1,board_0_I3,board_0_I2,board_0_I1 | user_app,user_consistency_error_detection_test | ? | seen(Machine(g_types)),imported(Machine(user_ctx)),imported(Machine(inputs)),imported(Machine(logic)),imported(Machine(outputs)) | ? | user_component_i);
...
THEORY ConstantsEnvX IS
  Constants(Implementation(user_component_i)) == (Type(pi) == Cst(btype(INTEGER,?,?));Type(lv) == Cst(btype(INTEGER,?,?));Type(av) == Cst(btype(INTEGER,?,?));Type(SimSMovement_cycle_def) == Cst(btype(INTEGER,?,?));Type(cycle_unit) == Cst(btype(INTEGER,?,?)))
END
&
