...
THEORY ListPropertiesX IS
  Abstract_List_Properties(Implementation(user_ctx_i))==(cycle_unit: uint32_t & SimSMovement_cycle_def: uint32_t & av: uint32_t & lv: uint32_t & pi: uint32_t & av: 0..3 & lv: 0..3);
...
THEORY ListValuesX IS
  ...
  Values_Subs(Implementation(user_ctx_i))==(cycle_unit,SimSMovement_cycle_def,av,lv,pi: 100,1,3,3,7000);
  List_Values(Implementation(user_ctx_i))==(cycle_unit = 100;SimSMovement_cycle_def = 1;av = 3;lv = 3;pi = 7000)
END
&
...
THEORY InheritedEnvX IS
  Constants(Implementation(user_ctx_i))==(Type(cycle_unit) == Cst(btype(INTEGER,?,?));Type(SimSMovement_cycle_def) == Cst(btype(INTEGER,?,?));Type(av) == Cst(btype(INTEGER,?,?));Type(lv) == Cst(btype(INTEGER,?,?));Type(pi) == Cst(btype(INTEGER,?,?)))
END
&
...
THEORY ConstantsEnvX IS
  Constants(Implementation(user_ctx_i)) == (Type(pi) == Cst(btype(INTEGER,?,?));Type(lv) == Cst(btype(INTEGER,?,?));Type(av) == Cst(btype(INTEGER,?,?));Type(SimSMovement_cycle_def) == Cst(btype(INTEGER,?,?));Type(cycle_unit) == Cst(btype(INTEGER,?,?)))
END
&
THEORY TCIntRdX IS
