...
THEORY ListConstantsX IS
  List_Valuable_Constants(Implementation(user_ctx_i))==(linear,angular);
  Inherited_List_Constants(Implementation(user_ctx_i))==(linear,angular);
  List_Constants(Implementation(user_ctx_i))==(?)
END
&
THEORY ListSetsX IS
  Set_Definition(Implementation(user_ctx_i),STATE)==({NONE,SMovingForward,SObserving,DObserving,STurning,DTurning,Waiting,EObserving,SCollision});
...
THEORY ListPropertiesX IS
  Abstract_List_Properties(Implementation(user_ctx_i))==(linear: uint8_t & angular: uint8_t & STATE: FIN(INTEGER) & not(STATE = {}));
...
THEORY ListValuesX IS
  ...
  Values_Subs(Implementation(user_ctx_i))==(linear,angular: 2,2);
  List_Values(Implementation(user_ctx_i))==(linear = 2;angular = 2)
END
&
...
&
THEORY SetsEnvX IS
  Sets(Implementation(user_ctx_i)) == (Type(STATE) == Cst(SetOf(etype(STATE,0,8))))
END
&
THEORY ConstantsEnvX IS
  Constants(Implementation(user_ctx_i)) == (Type(angular) == Cst(btype(INTEGER,?,?));Type(linear) == Cst(btype(INTEGER,?,?));Type(SCollision) == Cst(etype(STATE,0,8));Type(EObserving) == Cst(etype(STATE,0,8));Type(Waiting) == Cst(etype(STATE,0,8));Type(DTurning) == Cst(etype(STATE,0,8));Type(STurning) == Cst(etype(STATE,0,8));Type(DObserving) == Cst(etype(STATE,0,8));Type(SObserving) == Cst(etype(STATE,0,8));Type(SMovingForward) == Cst(etype(STATE,0,8));Type(NONE) == Cst(etype(STATE,0,8)))
END
&
THEORY TCIntRdX IS
